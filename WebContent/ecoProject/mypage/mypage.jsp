...
<%	
	...
	int userActCount = dao.getActCount(userID);
	if(	userActCount > 0){
		articleList = dao.getAct(userID, actStartRow, actEndRow);
	}
	
	System.out.println("회원 참여중 목록 " + userActCount);

//------------------------------------------------------------------------
	
	//01.완료된 활동 페이징 변수생성
	int endPageSize = 3; //한 페이지당 들어갈 컨텐트 숫자
	String endPageNum = request.getParameter("endPageNum");
	if (endPageNum == null) {
		endPageNum = "1";
	}

	int currentendPage = Integer.parseInt(actPageNum);
	int endStartRow = (currentActPage - 1) * actPageSize + 1;
	int endEndRow = currentActPage * actPageSize;
	int userEndActCount = dao.getEndActCount(userID);
	if(	userEndActCount > 0){
		endList = dao.getActend(userID, endStartRow, endEndRow);
	}
	
	System.out.println("완료활동 카운트 : " + userEndActCount);
	...
	//01.완료된 활동 페이징 변수생성
	int userTpcCount = dao.getTopCount(userID);
	int tpcPageSize = 3; //한 페이지당 들어갈 컨텐트 숫자
	String tpcPageNum = request.getParameter("tpcPageNum");
	if ( tpcPageNum == null) {
		tpcPageNum = "1";
	}

	int tpcCurrentendPage = Integer.parseInt(actPageNum);
	int tpcStartRow = (currentActPage - 1) * actPageSize + 1;
	int tpcSEndRow = currentActPage * actPageSize;
	if(	userTpcCount > 0){
		List topList = dao.getTop(userID, tpcStartRow, endStartRow);
	}
		...
		<div class="bod2">
			<div class="main_content">
				...
				<div class="mypageActivity">
					...
					<%
					if (userEndActCount > 0) {
						int endPageBlock = 5; //보여줄 페이지 숫자 1, 2, 3
						int endPageCount = userEndActCount / endPageSize + (userEndActCount % endPageSize == 0 ? 0 : 1);
						int endStartPage = (int) ((currentActPage - 1) / endPageBlock) * endPageBlock + 1;
						int endEndPage = endStartPage + endPageBlock - 1;
						if (endEndPage > endPageCount) endEndPage = endPageCount;
						if (endStartPage > endPageBlock) {%>
							<a href="mypage.jsp?endPageNum=<%=endStartPage - endPageBlock%>&pageNum=<%=pageNum%>">&lt;</a>
						<%}
						for (int i = endStartPage; i <= endEndPage; i++) {%>
							<a href="mypage.jsp?pageNum=<%=pageNum%>&endPageNum=<%=i%>">&nbsp;<%=i%>&nbsp;</a>
						<%}
						if (endEndPage < endPageCount) {%>
							<a href="mypage.jsp?pageNum=<%=pageNum%>&endPageNum=<%=endStartPage + endPageBlock%>">&gt;</a>
						<%}
					}%>
				</div>
					...
					<%-- 좋아요 누른 게시물들 테이블 --%>
				<%
					List contentList = udao.getLikeContent(userID, likeConStartRow, likeConEndRow);
					int likeContentCount = udao.getLikeContentCount(userID);
				%>
			...
			<%  } %>
						<!-- 내가 작성한 게시물 -->
							<%
							List myContentList = udao.getMyContents(userID, myConStartRow, myConEndRow);
							int myContentCount = udao.getMyContentCount(userID);
								%>
								<div class="main_content2">
									<div class="mypageLike">
									<h3>내가 작성한 게시물</h3>
				...
				<!-- 내가 작성한 댓글 -->
					<%
							List myCommentList = udao.myCommentList(userID, myComStartRow,myComEndRow);
							int myCommentCount = udao.myCommentCount(userID);
								%>
								<div class="main_content2">
									<div class="mypageLike">
									<h3>내가 작성한 댓글</h3>
								...
								<div class="gc-actnum">
									...
									if (myCommentCount > 0) {
										...
										if (myComStartPage > myComPageBlock) {%>
											<a href="mypage.jsp?myConPageNum=<%=myComStartPage - myComPageBlock%>&pageNum=<%=pageNum%>">&lt;</a>
										<%}
										for (int i = myComStartPage; i <= myComEndPage; i++) {%>
											<a href="mypage.jsp?pageNum=<%=pageNum%>&myConPageNum=<%=i%>">&nbsp;<%=i%>&nbsp;</a>
										<% }
										if (myComEndPage < myComPageCount) {%>
											<a href="mypage.jsp?pageNum=<%=pageNum%>&myConPageNum=<%=myComStartPage + myComPageBlock%>">&gt;</a>
										<% }
									}%>
								</div>	
						</form>
