...
//세션에서 id 가져오기
 ...
 /*주석처리*/
	System.out.println("1번 로그인중인 아이디 확인용 출력 :"+userID); //확인용
...
<%
	...
	System.out.println("19 번 리뷰리스트 출력 확인용1 :"+revList);
	if(count4 >0){
		revList=dao.getRevlist(userID, startRow4, endRow4);
	}
	System.out.println("19 번 리뷰리스트 출력 확인용2 :"+revList);//확인. 
	//474
	...
	System.out.println("30번  좋아요 출력 확인용 :"+likeList);
...
<%} %>		
		...
		<div class="body">
			<div class="main_content">
							...
							<tr>
								...
								<%	//이미지가 없을경우
									if(imgs == null || imgs == ""){
										...
										System.out.println("11번 null 이미지 확인용 출력 : " + imgs);// 이미지 이름 null 확인용 출력
								%>
									<%--<h3>찾는 이미지가 없습니다</h3>
									디폴트 이미지 줄것--%>
								...
								<%	}else{//이미지가 있다면 
										System.out.println("11번 이미지 이름 확인용 출력 : " + imgs);// 이미지 이름 확인용 출력
								%>
				...
				<%System.out.println("20번 회원정보 잘 가져왔는지 출력 확인용: " + userid.getName() ); %>
				...
				<div class="main_content">
					<form><h3>참여중인 활동 </h3>
						...
						if(articleList!=null){
							...
							System.out.println("15번 확인용 articleListsize :"+articleList.size()); 
							%>
							...
							<% //3개 이하일때
								for(int i = 0; i < articleList.size(); i++){
									...
									System.out.println("4번 article 출력 : "+ article);//확인용 출력
							%>
								...
								<tr>
									...
									<%	//이미지가 없을경우
										if(imgs == null || imgs == ""){
											...
											System.out.println("16번 null 이미지 확인용 출력 : "+imgs);// 이미지 이름 null 확인용 출력
									%>
									
										<%--<h3>찾는 이미지가 없습니다</h3>
										  
										디폴트 이미지 줄것--%>
										<img src="/ecopang/imgs/proImg.png" width="25"/>
											
									<%	}else{//이미지가 있다면 
								
									System.out.println("16번 이미지 경로  확인용 출력 : " + imgs);// 이미지 이름 확인용 출력
						...
						<%}else{//else%>
							<table>
								<tr>
									<td><pre><h4>참여중인 활동이 없습니다 ! 활동을 참여해 보세요 </h4></pre></td>
								<tr>
							</table>
						<%}%>
					</form>
					...
					<form>
						<% for(int i = 0; i < endList.size(); i++){
								ActDTO endArticle = (ActDTO)endList.get(i); 
								 if(endArticle != null){
								//System.out.println("8번 endArticle 출력 : "+ endArticle);//확인용 출력
						%><%-- ?  --%>
						<table border="1" style="float:left;">
						<tr>
					...
					<div>
						<table align="center">
							...
							<% if(count > 0) {
									...
									<div class="li-btn">
							            <a class="icon-prev" href="mypage.jsp?pageNum=<%=startPage-pageBlock %>" >
							                <span></span>
							            </a>
							        </div>
								<%	}
										...
										for(int i = startPage; i <= endPage; i++){
											...
											//System.out.println("17번 endArticle 출력 : "+ endArticle);//확인용 출력
									%> 
							            <div class="li">
							                <%--<a class="a dot" href="/ecopang/ecoProject/mypage/test99.jsp?pageNum=<%=currentPage+1%>"></a>--%>
							            	
							            	<a href="mypage.jsp?pageNum=<%=i%>" class="a dot"><%=i %></a>
							            </div>
									
									<%}// 오른쪽 화살표 
									if(endPage > pageCount) { %>
										<div class="li-btn">
							                <a class="icon-next" href="mypage.jsp?pageNum=<%=startPage+pageBlock%>" ></a>
							                    <span></span>
							            </div>
									<%}
								}//if(count > 0)%>
				...
				<div class="main_content">
					...
					<form>
						...
						<table>
							...
							<tr>
								<td><%=gto.getGroup_img()%></td>
							</tr>
								<%}else{ %>
							<tr>
								<td><img src="/ecopang/ecoProject/imgs/ecopang.png" width="50px"/></td>
							</tr>	
								<% } %>
							<tr>
					...
					<%if(revList != null){ %>
						<% for(int i = 0; i < revList.size(); i++){
							...
							ReviewDTO reviewArticle = (ReviewDTO)revList.get(i); 
								...
								//System.out.println("22번 revArticle 출력 : "+ reviewArticle);//확인용 출력
						%>
								<table border="1" style="float:left;">
									<tr>
									...
									<tr>
										<td><%
										String reContent =reviewArticle.getRev_content();
										if(reContent.length() >20 ){%>
											<%=reContent.substring(0,20) + "..."%>
										<%}else{%>
											<%=reContent%>&nbsp;
										<%}
										%>
					...
					<%if(topList != null){ %>
						<% for(int i = 0; i < topList.size(); i++){
							...
							TopicDTO topArticle = (TopicDTO)topList.get(i); 
								 if(topArticle != null){
								System.out.println("22번 topArticle 출력 : "+ topArticle);//확인용 출력
						%>
								<table border="1" style="float:left;">
									<tr>
										<td><h5><%=topArticle.getTpc_title() %>&nbsp;</h5></td>
									<tr>
										<td><%String content =topArticle.getTpc_content();
										if(content.length() > 20) { %>
										<%= content.substring(0,20) + "..." %>
									<% } else { %>
										<%= content %>&nbsp;
									<% } %>
										<%--<%=topArticle.getTpc_content() --%></td>
									</tr>
								</table>
							 <%} 
							}%>
					...
					<div>
						<table>
							...
							<% if(count2 > 0) {
									...
									<div class="li-btn">
							            <a class="icon-prev" href="mypage.jsp?pageNum2=<%=startPage2-pageBlock2%>&pageNum4=<%=startPage2-pageBlock2%>" >
							                <span></span>
							            </a>
							        </div>
								 <%}								
									...
									%> 
							            <div class="li">
							            	<a href="mypage.jsp?pageNum2=<%=i%>&pageNum4=<%=i%>" class="a dot"><%=i %></a>
							            </div>
									<%}// 오른쪽 화살표 
										if(endPage2 < pageCount2) { %>
										<div class="li-btn">
										&nbsp; <a href="mypage.jsp?pageNum2=<%=startPage2+pageBlock2%>&pageNum4=<%=startPage2+pageBlock2%>" class="pageNums"> &gt; </a>
										</div>
									<%	}
								}%>
							 	   </td>
					...
					<form>
						<% for(int i = 0; i < comList.size(); i++){
							...
							TopicCommentDTO comArticle = (TopicCommentDTO)comList.get(i); 
								 if(comArticle != null){
								System.out.println("28번 comArticle 출력 : "+ comArticle);//확인용 출력
								System.out.println("29번 comArticle.getTpc_num 출력 : "+comArticle.getTpc_num());
						%>
						<table border="1" style="float:left;">
						<tr>
					...
					<div class="main_content">
						<table align="center">
							...
							<% if(count3 > 0) {
									...
									<div class="li-btn">
							            <a class="icon-prev" href="mypage.jsp?pageNum3=<%=startPage3-pageBlock3%>" >
							                <span></span>
							            </a>
							        </div>
								<%	}								
									...
									%> 
							            <div class="li">
							            	<a href="mypage.jsp?pageNum3=<%=i%>" class="a dot"><%=i %></a>
							            </div>
									<%}// 오른쪽 화살표 
									if(endPage3 > pageCount3) { %>
										<div class="li-btn">
							                <a class="icon-next" href="mypage.jsp?pageNum3=<%=startPage3+pageBlock3%>" ></a>
							                    <span></span>
							            </div>
								<%}
								}%>
