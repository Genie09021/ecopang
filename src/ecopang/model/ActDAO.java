...
public class ActDAO {	
	...
	public void insertAct(ActDTO dto) {
	...
	public void updateActivity(ActDTO activity) {
...
//	//활동 고유번호 받기
	...
	// 삭제
	public void deleteActivity(ActDTO activity) {
		activity.setUserID("USER01");
		Connection conn = null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		...
		try {
			...
			if(rs.next()) {
				String dbuserId =rs.getString("userId");
				if(dbuserId.equals(activity.getUserID())){
					sql = "delete from activities where act_num=? and userId=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, activity.getAct_num());	
					pstmt.setString(2, activity.getUserID());	
					result = pstmt.executeUpdate();
				}
			}
		}catch(Exception e) {
