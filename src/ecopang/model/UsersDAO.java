...
public class UsersDAO {
	...
	// 아이디 중복 여부 확인 메서드
		...
		public List getAct(String userID, int startRow, int endRow) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			...
			try {
				String sql = "select * from (select C.*, rownum r from " + 
						...
						"where (A.act_num = B.act_num) and B.userID like ? order by act_date desc)C " + 
...
//-----------------			
		...
		public int getEndActCount(String userID) {
			int count = 0;
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			...
			try {
				conn= getConnection();
				String sql = "select count(*) from (select C.*, rownum r from " + 
						"(select B.* from activityjoin A, activities B " + 
...
*/		
		...
		public List getActJoin(String userID) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;//sql문 실행 결과를 받아올 ResultSet타입의 rs변수 미리선언
			List list = null;//담을 리스트 변수 필요
			...
			try {
				String sql = "select act_num from activityjoin where userId = ?";
				conn = getConnection();
				pstmt=conn.prepareStatement(sql);
				pstmt.setString(1, userID);
				rs = pstmt.executeQuery();
				...
				if(rs.next()) {
					...
					do {//확인용 rs.next()로 한칸 이미 내려간 상태 
						//	->바로 do~while문 사용
						ActivityJoinDTO dto = new ActivityJoinDTO(); //activites 에 얻은 값 셋팅 해주기
						dto.setAct_Num(rs.getInt("act_num"));
						dto.setUserID(rs.getString("userID"));
						
						list.add(dto);//리턴 해줄 데이터
		...
		public List getLikeContent(String userID, int startRow, int endRow) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;//sql문 실행 결과를 받아올 ResultSet타입의 rs변수 미리선언
			List list = null;//담을 리스트 변수 필요
		...
		public int getLikeContentCount(String userID) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;//sql문 실행 결과를 받아올 ResultSet타입의 rs변수 미리선언
		...
		//내가 쓴 글 목록 불러오기
		public List getMyContents(String userID, int startRow, int endRow) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;//sql문 실행 결과를 받아올 ResultSet타입의 rs변수 미리선언
			List list = null;//담을 리스트 변수 필요
		...
		//내가 쓴 글 카운트
		public int getMyContentCount(String userID) {
			Connection conn = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;//sql문 실행 결과를 받아올 ResultSet타입의 rs변수 미리선언
			int count = 0;//담을 리스트 변수 필요
